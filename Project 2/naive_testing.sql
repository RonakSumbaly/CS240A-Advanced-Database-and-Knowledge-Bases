...
-- create tables 
CREATE TABLE TEST_RESULTS("NUM" INTEGER DEFAULT 1, "COLUMN" INTEGER, "ACTUAL" VARCHAR(10), "PREDICTED" VARCHAR(10)); -- CREATE TABLE TO STORE CLASS LABELS FOR TEST DATASET
CREATE TABLE COMBINED_RESULTS("NUM" INTEGER DEFAULT 1, "ACCURACY" DECIMAL(11,10)); -- TEST THE ACCURACY

INSERT INTO TEST_RESULTS(COLUMN, ACTUAL, PREDICTED)
	WITH -- USAGE OF THREE TABLE TO GET THE PROBABILITY, WEIGHTS AND MAX PROBABILITY OF EACH RECORDS IN THE TEST DATASET
	...
	AND TD.COLUMNNO = 1); -- ASSIGN ONE IF CORRECTLY PREDICTED ELSE 0
...
INSERT INTO COMBINED_RESULTS(ACCURACY)
	WITH
		...
		(SELECT CAST(SUCCESS.num AS DECIMAL(10,0))/CAST(TOTAL.num AS DECIMAL(10,0)) - CALCULATE THE \% of CORRECTLY CLASSIFIED LABELS
		FROM SUCCESS, TOTAL);
